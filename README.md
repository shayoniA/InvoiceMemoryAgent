...
1. **Vendor-Specific Memory** - Agent maintains long-term memory per vendor, such as Common field patterns and Historical correction confidence.
...
3. **Conservative Learning (Safety-First)** - The agent does not blindly learn from every invoice. Learning happens only if:
	 Invoice is not a duplicate,
	 Corrections are applied successfully,
	 Confidence exceeds a safe threshold.

4. **Explainable Decisions & Audit Trail** - Every run produces a full audit trail:  Pattern detection --> Memory recall --> Applied corrections --> Final decision.
Example audit steps:
{
  "step": "apply",
...
4. **Memory Recall (Vendor-Scoped)**: src/memory/memoryStore.ts - Memory is recalled using vendor + memoryType + detected patterns. Example of memory structure:
{
...
6. **Memory Application (Auto-Correction)**: src/index.ts - If a field is missing / a matching memory exists / confidence is high, then the agent applies a correction, example: Filled serviceDate using vendor memory (Leistungsdatum).

7. **Confidence Scoring**: src/decision/decide.ts - Final confidence is computed using - Base invoice confidence, Memory confidence boosts, Penalties for uncertainty.
Example:  *0.78 (base) + 0.05 (memory boost) = 0.83*

8. **Decision Gate (Human-in-the-Loop)**: Automation based on final confidence - *requiresHumanReview = confidence < 0.75* (threshold)

9. **Learning & Memory Update (Controlled)**: src/memory/memoryStore.ts - Memory is updated iff - Invoice is not duplicate, Correction was applied, Confidence is high.  *timesUsed++; timesApproved++; confidence += small_delta*

10. **Audit Trail Generation**: src/audit/auditTrail.ts - Audit steps include: recall --> apply --> decide --> learn. Every step is recorded.
